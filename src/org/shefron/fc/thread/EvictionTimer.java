 ...
 * the standard {@link Timer} and keeps track of how many pools are using it. If
 * no pools are using the timer, it is canceled. This prevents a thread being
 * left running which, in application server environments, can lead to memory
...
class EvictionTimer {// Öð³ö,¸Ï³ö
	...
	static synchronized void schedule(TimerTask task, long delay, long period) {
		if (null == _timer) {
			...
			ClassLoader ccl = AccessController
	 ...
	 */
	private static class PrivilegedSetTccl implements
			PrivilegedAction<ClassLoader> {
