...
/**
 * This utility wrapper class is created to support AXIS2 runs inside of Java 2
 * Security environment. Due to the access control checking algorithm, for Java
 * 2 Security to function properly, <code>doPrivileged()</code> is required in
 * cases where there is application code on the stack frame accessing the system
 * resources (ie, read/write files, opening ports, and etc). This class also
 * improve performance no matther Security Manager is being enabled or not.
 * <p/>
...
public class AccessController {
	 ...
	 * domains represented by the specified <code>AccessControlContext</code> if
	 ...
	 *            the action to be performed.
	 * @return the value returned by the action's <code>run</code> method.
	 * @throws PrivilgedActionException
	 *             the specified action's <code>run</code> method threw a
	 ...
	 * intersection of the the permissions possessed by the caller's protection
	 ...
	 * currently in effect. This method quietly returns if the access request is
	 * permitted, or throws a suitable AccessControlException otherwise.
	 * 
	 * @param perm
